"""
NVIDIA Nemotron-3-Embed-8B-BF16 dense retrieval baseline for patent-product matching.
"""
...
def run(
    ...
    """Nemotron-3-Embed-8B-BF16 dense retrieval baseline."""
    print("\nEXPERIMENT: NEMOTRON-3-EMBED-8B-BF16")
    ...
    # Encode full product corpus as passages. encode_document() applies
    # the model's document-side prompt internally
    print(f"Encoding {len(all_product_ids)} products...")
    product_texts = [
        get_product_text(products[pid], masked=masked) for pid in all_product_ids
    ]
    product_embeddings = model.encode_document(
        product_texts,
    ...
    # Encode all patents as queries. encode_query() applies the model's
    # query-side task instruction internally
    ...
    patent_embeddings = model.encode_query(
        patent_texts,
        batch_size=batch_size,
        normalize_embeddings=True,
        show_progress_bar=True,
